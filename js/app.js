import { parseLogs, COST_PER_EXACT_GRAM } from './parser.js';
import { saveRawLogs, loadRawLogs, saveMyExpenses, loadMyExpenses, clearStorage } from './store.js';
  ...
  container.innerHTML = myExpenses.slice().reverse().map(item => `
    <div class="flex justify-between items-center bg-brandDark/60 p-1.5 rounded-lg border border-brandBorder">
      ...
      <div class="flex items-center gap-1.5 shrink-0">
        ...
        <button data-id="${item.id}" class="btn-delete-my text-gray-500 hover:text-red-400 font-bold px-1">×</button>
      </div>
    </div>
  `).join('');

  document.querySelectorAll('.btn-delete-my').forEach(btn => {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('btnTodayWeek').addEventListener('click', resetToCurrentWeek);
...
function addMyExpense(type) {
  ...
  amount = type === 'sub' ? -Math.abs(amount) : Math.abs(amount);
